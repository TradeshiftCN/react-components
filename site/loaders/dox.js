  ...
  const callbacks = [
    ...
    match => {
      ...
      if (_imports) {
        ...
        currentDemo.mainCode = currentDemo.code
          .replace(/import .*/g, '')
          .trim();
      }
      currentDemo.name = currentDemo.mainCode.match(
        /(?:const|class)\s(\w+)/
      )[1];
    },
    match => {
      ...
      if (match) {
        ...
        while ((res = reg.exec(match))) {
          ...
          let type = res[4].match(
            /shape|string|bool|number|object|array|func|node|element|oneOf.*/g
          )[0];
